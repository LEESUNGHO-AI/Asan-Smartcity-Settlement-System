...
function 단위서류(x) {
  ...
  // 계약금액이 없으면 해당 단위사업 집행액으로 대체 표시
  const 집행액 = evidence.filter((e) => e.단위사업 === x.단위사업).reduce((s2, e) => s2 + e.집행금액, 0);
  const 금액표시 = ct.계약금액 ? ct.계약금액.toLocaleString('ko-KR') + '원'
    : (집행액 ? 집행액.toLocaleString('ko-KR') + '원 (집행액 기준)' : '');
  const 공통 = [
    ['사 업 명', base.사업.사업명],
    ['단위사업', subName(x.단위사업)],
    ['공 사 명', x.명칭],
    ['계약상대자', x.수행사 || (ct.계약상대자 || {}).상호 || '(계약 확정 후 기재)'],
    ['계약번호', x.계약ID || '(계약 확정 후 기재)'],
    ['계약금액', 금액표시],
    ['계약일자', ct.계약일 ? kdate(ct.계약일) : '(계약 확정 후 기재)'],
    ['준공기한', ct.준공예정일 ? kdate(ct.준공예정일) : (x.비고 || '').replace('준공예정 ', '')],
  ];
  const files = [];
  ...
  // ── 검수조서 ──
  {
    ...
    const 검수행 = 자산목록.length
      ? 자산목록.map((a, i) => [
          ...
          { t: '적합', align: 'c' }])
      : [[{ t: '자산관리 시스템에 등록된 취득자산이 없습니다. 취득 후 자동 반영됩니다.', align: 'c', span: 6, color: '888888' }]];
    // 취득가액 합계행
    if (자산목록.length) {
  ...
  // ── 시설물 인계·인수서 ──
  {
    ...
    const 인계행 = 인계자산.length
      ? 인계자산.map((a, i) => [
          ...
          `${a.규격 || ''} / ${a.수량 || 1}`, a.설치장소 || '',
          { t: a.자산ID, align: 'c' }])
      : [[{ t: '자산관리 시스템에 등록된 인계 자산이 없습니다. 취득 후 자동 반영됩니다.', align: 'c', span: 5, color: '888888' }]];
