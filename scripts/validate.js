...
function validateRules() {
  ...
  // ── R-07 재원비율 이탈 ───────────────────────────────
  const 확정 = ev.filter((e) => e.검토상태 === '확정' && e.재원 !== '미구분');
  const 재원합 = {};
  let 총합 = 0;
  for (const e of 확정) {
  ...
  // ── R-24 재원 미구분 ─────────────────────────────────
  const 미구분 = ev.filter((e) => e.재원 === '미구분');
  if (미구분.length) {
    const s3 = 미구분.reduce((a, e) => a + e.집행금액, 0);
    error('R-24', `재원 미구분 ${미구분.length}건`,
      `${won(s3)} 의 재원(국비·도비·시비) 구분이 없음 — 정산보고서는 재원별 구분 작성이 필수`, '작성지침 §4②');
  }
